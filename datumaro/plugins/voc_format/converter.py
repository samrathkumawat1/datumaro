...
def _write_xml_bbox(bbox, parent_elem):
    ...
    ET.SubElement(bbox_elem, 'x-axis').text = str(x1)
    ET.SubElement(bbox_elem, 'y-axis').text = str(y1)
    ET.SubElement(bbox_elem, 'z-axis').text = str(z1)
    return bbox_elem
                ...
                if self._tasks is None and bboxes or \
                    ...
                    for new_obj_id, obj in enumerate(main_bboxes):
                        ...
                        if self._allow_attributes:
                            ...
                            if obj_label == "EgoVehicle":
                                attr_elem = ET.SubElement(attrs_elem, 'attribute')
                                gps_elem = ET.SubElement(attr_elem, 'GPSData')
                            print("--attr-->",attr);
                            print("values--->",obj_label)
                            for k, v in attr.items():
                                ...
                                if k != "latitude" and k != "longitude":
                                    ...
                                    if k=="latitude":
                                        ET.SubElement(gps_elem, 'latitude').text = str(v)
                                    elif k=="longitude":
                                        ET.SubElement(gps_elem, 'longitude').text = str(v)
                   

                            #if len(gps_elem):
                            #   attrs_elem.append(gps_elem)
                            if len(attrs_elem):
