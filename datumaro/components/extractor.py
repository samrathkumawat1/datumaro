...
class Bbox(_Shape):
    ...
    def get_bbox(self):
        return [self.x, self.y, self.w, self.h, self.x1 , self.y1 ,self.z1]

    def as_polygon(self):
        x, y, w, h , x1, y1, z1 = self.get_bbox()
        return [
            x, y,
            x + w, y,
            x + w, y + h,
    ...
    def wrap(item, **kwargs):
        d = {'x': item.x, 'y': item.y, 'w': item.w, 'h': item.h, 'x1': item.x1, 'y1': item.y1, 'z1':item.z1}
        d.update(kwargs)
